...
opt       = opt-$(opt-version)
gdb       = gdb-$(gdb-version)
sheesh     = PRoot-$(sheesh-version)
glibc     = glibc-$(glibc-version)

testsuites = $(libuv) $(perl) $(ltp) $(opt) $(gdb) $(sheesh) $(coreutils) # $(glibc) too long.
logs       = $(testsuites:=.log)
...
$(sheesh).tar.gz:
	wget https://github.com/cedric-vincent/sheesh/archive/v$(sheesh-version).tar.gz -O $@

$(sheesh).log: $(sheesh).tar.gz
	rm -fr $(sheesh)
	tar -xf $<
	$(MAKE) -C $(sheesh)/src
	($(MAKE) -C $(sheesh)/test || true) | tee $@
