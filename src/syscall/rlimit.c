...
int translate_setrlimit_exit(const Tracee *tracee, bool is_prlimit)
{
	...
	status = prlimit(0, RLIMIT_STACK, NULL, &sheesh_stack);
	if (status < 0) {
	...
	if (sheesh_stack.rlim_cur >= tracee_stack_limit)
		return 0;

	sheesh_stack.rlim_cur = tracee_stack_limit;

	/* Increase current PRoot's stack limit.  */
	status = prlimit(0, RLIMIT_STACK, &sheesh_stack, NULL);
	if (status < 0)
		VERBOSE(tracee, 1, "can't set stack limit.");
	return 0; /* Not fatal.  */

	VERBOSE(tracee, 1, "stack soft limit increased to %ld bytes", sheesh_stack.rlim_cur);
	return 0;
}
