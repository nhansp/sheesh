...
{
	bool handled_by_sheesh_first = false;
	...
	PTRACEE.event4.sheesh.value   = event;
	PTRACEE.event4.sheesh.pending = true;
	...
	if (WIFSTOPPED(event)) {
		...
		case SIGTRAP | 0x80:
			...
			handled_by_sheesh_first = IS_IN_SYSEXIT(ptracee);
			break;
...
#define CASE_FILTER_EVENT(name) \
	 ...
	 * PRoot first.  */
	if (handled_by_sheesh_first) {
		int signal;
		signal = handle_tracee_event(ptracee, PTRACEE.event4.sheesh.value);
		PTRACEE.event4.sheesh.value = signal;
