...
static int archive_re_execute_sh(Care *care)
{
	...
	N("PROOT=\"${PROOT-$(dirname $0)/sheesh}\"");
	N("");

	N("if [ ! -e ${PROOT} ]; then");
	N("    PROOT=$(which sheesh)");
	N("fi");
	N("");

	N("if [ -z ${PROOT} ]; then");
	N("    echo '**********************************************************************'");
	N("    echo '\"sheesh\" command not found, please get it from https://sheesh-me.github.io'");
	N("    echo '**********************************************************************'");
	N("    exit 1");
	N("fi");
	N("");
...
static int archive_readme_txt(const Care *care)
{
	...
	N("This archive was created with CARE: https://sheesh-me.github.io. It contains:");
	N("");
	N("re-execute.sh");
 ...
 * "@care->archive:@care->prefix/sheesh".  Note: this function is
...
static int archive_myself(const Care *care)
{
	...
	location = talloc_asprintf(care, "%s/sheesh", care->prefix);
	if (location == NULL) {
		note(NULL, ERROR, INTERNAL, "can't allocate location for 'sheesh'");
		return -1;
	}
...
/**
 * Archive "re-execute.sh" & "sheesh" from @care.  This function
...
#if defined(CARE_BINARY_IS_PORTABLE)
	/* Archive "care" as "sheesh", these are the same binary. */
	status = archive_myself(care);
	if (status < 0)
		note(NULL, WARNING, INTERNAL, "can't archive 'sheesh'");
#endif
