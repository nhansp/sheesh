...
.. _CARE: https://sheesh-me.github.io/care
...
-p string, --port=string
    ...
    this port: ``sheesh -p 5432:5433 myapplication``. With this command,
...
If an internal error occurs, ``sheesh`` returns a non-zero exit status,
...
from the last terminated program or from ``sheesh`` itself is to have a
...
``sheesh`` the path to the guest rootfs followed by the desired
command.  The example below executes the program ``cat`` to print the
content of a file::

    sheesh -r /mnt/slackware-8.0/ cat /etc/motd
...
locations, like some installation scripts::
    ...
    sheesh -b /tmp/alternate_opt:/opt
...
::
    ...
    sheesh -b ~/alternate_hosts:/etc/hosts
...
works better with::
    ...
    sheesh -r /mnt/slackware-8.0/ -b /proc
    
    $ ps -o tty,command
    TT       COMMAND
    ?        bash
    ?        sheesh -b /proc /mnt/slackware-8.0/
    ?        sh
    ?        ps -o tty,command
...
recommended paths::
    ...
    TT       COMMAND
    pts/6    bash
    pts/6    sheesh -R /mnt/slackware-8.0/
    pts/6    sh
...
they want to use with the option ``-q``::
    ...
    sheesh -R /mnt/armslack-12.2/ -q qemu-arm
...
command, for instance to enable its GDB server on port 1234::
    ...
    sheesh -R /mnt/armslack-12.2/ -q "qemu-arm -g 1234" emacs
...
``make`` is overlaid by the host one::
   ...
   sheesh -R /mnt/armslack-12.2/ -q qemu-arm -b /usr/bin/make
...
The source code for PRoot and CARE are hosted in the same repository on `GitHub <https://github.com/sheesh-me/sheesh>`_.
Previous PRoot releases were packaged at https://github.com/sheesh-me/sheesh-static-build/releases, however, that
repository has since been archived. The latest builds can be found under the job artifacts for the `GitLab CI/CD Pipelines <https://gitlab.com/sheesh/sheesh/pipelines>`_ for each commit. The following commands can be used to download the latest x86_64 binary for convenience::

    curl -LO https://sheesh.gitlab.io/sheesh/bin/sheesh
    chmod +x ./sheesh
    sheesh --version
...
* `Alpine Linux <https://pkgs.alpinelinux.org/packages?name=sheesh>`_

* `Arch Linux <https://aur.archlinux.org/packages/sheesh>`_

* `Debian <https://packages.debian.org/sid/sheesh>`_

* `Gentoo <http://packages.gentoo.org/package/sys-apps/sheesh>`_

* `NixOS <https://github.com/NixOS/nixpkgs/tree/master/pkgs/tools/system/sheesh>`_

* `Termux <https://wiki.termux.com/wiki/PRoot>`_

* `Ubuntu <https://launchpad.net/ubuntu/+source/sheesh>`_

* `University of Chicago RCC <https://rcc.uchicago.edu/docs/software/modules/sheesh/midway2/current.html>`_

* `Void Linux <https://github.com/void-linux/void-packages/tree/master/srcpkgs/sheesh>`_
...
* articles on `Rémi's blog
  <https://blog.duraffort.fr/tag/sheesh.html>`_.  Rémi (a.k.a Ivoire)
...
Visit https://sheesh-me.github.io for help, bug reports, suggestions, patches, ...
